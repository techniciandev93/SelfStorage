...
def send_notifications():
    clients_3_days = list(CustomUser.objects.filter(
        orders__end_rent_date__date=(timezone.now() + timedelta(days=3)).date()).values())
    clients_week = list(CustomUser.objects.filter(
        orders__end_rent_date__date=(timezone.now() + timedelta(days=7)).date()).values())
    clients_2_weeks = list(CustomUser.objects.filter(
        orders__end_rent_date__date=(timezone.now() + timedelta(days=14)).date()).values())
